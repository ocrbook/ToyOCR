...
class CenterNetGT(object):
    ...
    def generate(config, batched_input, image_shape):
        ...
        box_scale = 1 / config.MODEL.DETNET.DOWN_SCALE
        num_classes = config.MODEL.DETNET.NUM_CLASSES
        H, W = image_shape  # config.MODEL.DETNET.OUTPUT_SIZE
        output_size = [int(H * box_scale), int(W * box_scale)]
        #output_size = config.MODEL.DETNET.OUTPUT_SIZE
        min_overlap = config.MODEL.DETNET.MIN_OVERLAP
        tensor_dim = config.MODEL.DETNET.TENSOR_DIM
        ...
        for data in batched_input:
            ...
            gt_index[:num_boxes] = centers_int[:num_boxes, 1] * \
                output_size[1] + centers_int[:num_boxes, 0]
            gt_reg[:num_boxes] = centers[:num_boxes, :] - \
                centers_int[:num_boxes, :]
            ...
            CenterNetGT.generate_score_map(
                gt_scoremap, classes[:num_boxes], wh, centers_int, min_overlap)
            gt_wh[:num_boxes] = wh
