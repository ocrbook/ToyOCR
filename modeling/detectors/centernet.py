import math
from modeling.necks.fpn import FPNDeconv
...
class CenterNet(nn.Module):
    ...
    def losses(self, pred_dict, gt_dict):
        ...
        if self.cfg.MODEL.DETNET.LOSS.IGNORE_UNLABEL:
            loss_cls = ignore_unlabel_focal_loss(pred_score, gt_dict["score_map"])
        elif self.cfg.MODEL.DETNET.LOSS.MSE:
        ...
        norm_wh =  self.cfg.MODEL.DETNET.LOSS.NORM_WH
        loss_wh = reg_l1_loss(pred_dict["wh"], mask, index, gt_dict["wh"], norm_wh)

        # regression loss
        loss_reg = reg_l1_loss(pred_dict["reg"], mask, index, gt_dict["reg"])

        loss_cls *= self.cfg.MODEL.DETNET.LOSS.CLS_WEIGHT
        loss_wh *= self.cfg.MODEL.DETNET.LOSS.WH_WEIGHT
        loss_reg *= self.cfg.MODEL.DETNET.LOSS.REG_WEIGHT
